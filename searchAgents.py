...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            flag = copy.copy(state[1])
            cornerList = list(flag)
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(corners)):
        if state[1][i] == False:
            mazeDist = mazeDistance(corners[i], state[0], problem.startingGameState)
            if (mazeDist > maxDist):
                maxDist = mazeDist
    return maxDist
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        foo.append(mazeDistance(position,food,problem.startingGameState))
    maxDist= max(foo)
    return maxDist
